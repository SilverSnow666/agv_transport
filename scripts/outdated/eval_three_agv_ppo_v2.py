    ...
    with open(summary_csv, "w", newline="", encoding="utf-8") as summary_file, open(
        ...
        summary_writer.writerow(
            [
                ...
                "agv1_action_rate_mean",
                "agv2_action_rate_mean",
                "agv3_action_rate_mean",
                "agv1_low_utility_action_mean",
                "agv2_low_utility_action_mean",
                "agv3_low_utility_action_mean",
                "episode_reward",
                "terminated",
                "truncated",
            ]
        ...
        while simulation_app.is_running() and global_step < args_cli.max_steps:
            ...
            with torch.inference_mode():
                ...
                prev_env_actions = get_env_actions(base_env, actions).clone()
                obs, rewards, terminated, truncated, info = env.step(actions)
                env_actions = get_env_actions(base_env, actions)
                ...
                push_utility, second_push_utility, top2_push_utility, two_pusher_gate, _ = compute_push_metrics(
                    base_env, payload_xy, contact_flags
                )
                action_norm, action_rate_norm = compute_action_metrics(env_actions, prev_env_actions)

                low_utility_threshold = getattr(base_env.cfg, "idle_low_utility_threshold", 0.08)
                low_utility_weight = torch.clamp(
                    (low_utility_threshold - push_utility) / max(low_utility_threshold, 1e-6),
                    min=0.0,
                    max=1.0,
                )
                ...
                min_yaw_abs_inside_target[position_success] = torch.minimum(
                    min_yaw_abs_inside_target[position_success],
                    payload_yaw_abs[position_success],
                )
                ...
                min_final_goal_dist[valid_env] = torch.minimum(
                    min_final_goal_dist[valid_env], final_goal_dist[valid_env]
                )
                payload_yaw_abs_max[valid_env] = torch.maximum(
                    payload_yaw_abs_max[valid_env], payload_yaw_abs[valid_env]
                )
                ...
                formation_error_max[valid_env] = torch.maximum(
                    formation_error_max[valid_env], formation_error_max_step[valid_env]
                )

                path_lateral_error_sum[valid_env] += path_lateral_error[valid_env]
                path_lateral_error_max[valid_env] = torch.maximum(
                    path_lateral_error_max[valid_env], path_lateral_error[valid_env]
                )
                path_progress_max[valid_env] = torch.maximum(
                    path_progress_max[valid_env], path_progress[valid_env]
                )
                ...
                env_actions_cpu = env_actions.detach().cpu()
                ...
                for env_id in range(num_envs):
                    ...
                    traj_writer.writerow(
                        [
                            ...
                            float(lookahead_target_dist[env_id].item()),
                            ...
                            float(action_norm[env_id, 0].item()),
                            float(action_rate_norm[env_id, 0].item()),
                            float(agv_positions[1][0].item()),
                            float(agv_positions[1][1].item()),
                            bool(contact_flags[env_id, 1].item()),
                            float(push_utility[env_id, 1].item()),
                            float(action_norm[env_id, 1].item()),
                            float(action_rate_norm[env_id, 1].item()),
                            float(agv_positions[2][0].item()),
                            float(agv_positions[2][1].item()),
                            bool(contact_flags[env_id, 2].item()),
                            float(push_utility[env_id, 2].item()),
                            float(action_norm[env_id, 2].item()),
                            float(action_rate_norm[env_id, 2].item()),
                            float(two_pusher_gate[env_id].item()),
                            float(second_push_utility[env_id].item()),
                            float(top2_push_utility[env_id].item()),
                            float(formation_errors[env_id, 0].item()),
                            float(formation_errors[env_id, 1].item()),
                            float(formation_errors[env_id, 2].item()),
                            float(env_actions_cpu[env_id, 0].item()),
                            float(env_actions_cpu[env_id, 1].item()),
                            float(env_actions_cpu[env_id, 2].item()),
                            float(env_actions_cpu[env_id, 3].item()),
                            float(env_actions_cpu[env_id, 4].item()),
                            float(env_actions_cpu[env_id, 5].item()),
                ...
                for env_id in range(num_envs):
                    ...
                    final_progress_ratio = float(path_progress_ratio[env_id].item())
                    max_progress_ratio = float(
                        torch.clamp(path_progress_max[env_id] / path_total_length[env_id], min=0.0, max=1.0).item()
                    )
                    ...
                    summary_writer.writerow(
                        [
                            ...
                            float(min_final_goal_dist[env_id].item()),
                            float(payload_yaw_abs_max[env_id].item()),
                            float(final_yaw_abs_for_summary),
                            float(target_yaw_radius),
                            ...
                            float(two_pusher_active_ratio.item()),
                            float(second_push_mean.item()),
                            float(top2_push_mean.item()),
                            ...
                            float(action_rate_mean[0].item()),
                            float(action_rate_mean[1].item()),
                            float(action_rate_mean[2].item()),
                            float(low_utility_action_mean[0].item()),
                            float(low_utility_action_mean[1].item()),
                            float(low_utility_action_mean[2].item()),
                    ...
                    print(
                        ...
                        f"final_yaw_abs={final_yaw_abs_for_summary:.3f}, "
                        f"path_lat_mean={float(path_lateral_mean.item()):.3f}, "
                        f"path_lat_max={float(path_lateral_error_max[env_id].item()):.3f}, "
                        f"progress_ratio={final_progress_ratio:.3f}, "
                        f"two_gate_mean={float(two_pusher_gate_mean.item()):.3f}, "
                        f"two_gate_active={float(two_pusher_active_ratio.item()):.3f}, "
                        f"contact=({float(contact_ratios[0].item()):.2f}, "
                        f"{float(contact_ratios[1].item()):.2f}, "
                        f"{float(contact_ratios[2].item()):.2f}), "
                        f"action_norm=({float(action_norm_mean[0].item()):.2f}, "
                        f"{float(action_norm_mean[1].item()):.2f}, "
                        f"{float(action_norm_mean[2].item()):.2f}), "
