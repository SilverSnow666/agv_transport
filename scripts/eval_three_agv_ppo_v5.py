"""Evaluate the current three-AGV policy for V5.2 physical-boundary tests.
...
parser = argparse.ArgumentParser(
    description="Evaluate three-AGV centralized PPO policy for V5.2 physical-boundary validation."
)
...
def compute_wall_clearances(base_env, payload_xy: torch.Tensor, agv_xy: torch.Tensor):
    ...
    payload_half_width = 0.5 * float(getattr(base_env.cfg, "payload_size", (0.90, 1.20, 0.30))[1])
            ...
            with torch.inference_mode():
                ...
                for env_id in range(num_envs):
                    ...
                    print(
                        ...
                        f"done_flags=(pos={row[36]}, yaw={row[37]}, oob={row[38]}, rear={row[39]}, escape={row[40]}), "
